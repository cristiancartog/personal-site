...
switch (process.argv[2]) {
    case 'start': // The "start" script is run during development mode
        rewireModule('react-scripts/scripts/start.js', loadCustomizer('./config-overrides.dev'));
        break;
    case 'build': // The "build" script is run to produce a production bundle
        rewireModule('react-scripts/scripts/build.js', loadCustomizer('./config-overrides.prod'));
        break;
    case 'test': // The "test" script runs all the tests with Jest
        break;
    default:
